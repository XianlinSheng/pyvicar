...
class FSIList(Group, Readable, Optional):
    def __init__(self, case):
        ...
        self._children.t1 = FSI(self._case, 1)
        self._children.t2 = FSI(self._case, 2)
    ...
    def latest(self):
        if not self:
            raise Exception(f"No active restart fsi out files")
...
class FSI(Group, Readable, Optional):
    ...
    def read(self):
        filepath = self._case.path / "Restart" / f"restart_fsi_out.{self._tidx}.dat"
    ...
    def to_restart_in(self):
        if not self:
            raise Exception(f"No active restart fsi out file")

        newpath = self._case.path / "Restart" / f"restart_fsi_in.dat"
        shutil.copy(self._path, newpath)
        return newpath

    def __repr__(self):
        return f"RestartFSI(tidx = {self._tidx})"
