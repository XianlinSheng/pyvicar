...
class RHMList(Group, Readable, Optional):
    def __init__(self, case):
        ...
        self._children.t1 = RHM(self._case, 1)
        self._children.t2 = RHM(self._case, 2)
    ...
    def latest(self):
        if not self:
            raise Exception(f"No active restart rhm out files")
...
class RHM(Group, Readable, Optional):
    ...
    def read(self):
        filepath = self._case.path / "Restart" / f"restart_rhm_out.{self._tidx}.dat"
    ...
    def to_restart_in(self):
        if not self:
            raise Exception(f"No active restart rhm out file")

        newpath = self._case.path / "Restart" / f"restart_rhm_in.dat"
        shutil.copy(self._path, newpath)
        return newpath

    def __repr__(self):
        return f"RestartRHM(tidx = {self._tidx})"
