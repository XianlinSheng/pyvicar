...
from pyvicar.tree.group import Group, List
from pyvicar.tree.field import Field
from pyvicar.file.io import Readable
from pyvicar.file.series import Series
...
class DragLiftList(List, Readable, Optional):
    ...
    def _elemcheck(self, new):
        if not isinstance(new, DragLift):
            raise TypeError(f'Expected a DragLift object inside DragLiftList, but encountered {repr(new)}')
