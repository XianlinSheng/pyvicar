...
class General(Group, Writable):
    def __init__(self, f):
        ...
        self._children.membraneType = Field('membraneType', 'open', '', {'open': 1, 'closed': 2, 'diff': 3})
              
        self._children.combinedType = Field('combinedType', 0)
        self._children.combinedGroupIndex = Field('combinedGroupIndex', 0)
        self._children.surfaceIntegral = Field('surfaceIntegral', True, '', Field.vmapPresets.bool2int)
              
        self._children.wallType = Field('wallType', 'noslip_nonporous', '', {'noslip_nonporous': 0, 'slip_porous': 1})
