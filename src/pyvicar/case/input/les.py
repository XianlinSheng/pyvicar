...
class LES(Group, Writable):
    def __init__(self, f):
        ...
        self._children.turbActive = Field(
            "turbActive", False, "", Field.vmapPresets.bool2int
        )

        self._children.turbModel = Field(
            "turbModel", 3, "", {"dynsmag": 1, "dynlagr": 2}
        )

        self._children.cSmagFix = Field(
            "cSmagFix",
            "turbulent",
            "for dynlagr init",
            {"laminar": 0.0, "turbulent": 0.1},
        )

        self._children.xTestFilter = Field(
            "xTestFilter", True, "", Field.vmapPresets.bool2int
        )
        self._children.yTestFilter = Field(
            "yTestFilter", True, "", Field.vmapPresets.bool2int
        )
        self._children.zTestFilter = Field(
            "zTestFilter", True, "", Field.vmapPresets.bool2int
        )
        ...
        self._children.turbulentChannel = Field(
            "turbulentChannel", False, "", Field.vmapPresets.bool2int
        )
