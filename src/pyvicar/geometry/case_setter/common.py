import numpy as np
import pandas as pd
from pyvicar.geometry.presets import (
    create_sphere,
    create_cyl_2d,
    create_plane,
    create_plane_2d,
...
def append_solid(case, mesh=None):
    ...
    nmemb = case.canonicalBody.nBodyMembrane.value
    if nmemb > 0:
        raise RuntimeError(
            f"Solid needs to be placed before all membranes, got existing {nmemb} membrane(s) already"
        )
...
def append_membrane(case, mesh=None, closed=False):
    case.input.ib.iib = True
    ...
    body = case.canonicalBody.bodies.appendnew()
    body.general.bodyType = "unstruc"
    body.general.membraneType = "closed" if closed else "open"
    body.general.nPoint = nPoint
    body.general.nElem = nElem
...
# membrane is generally open, so default non-cycled
def append_membrane_2d(case, curv, cycled=False):
    nz = case.input.domain.nz.value
    zout = case.input.domain.zout.value
    dz = zout / (nz - 1)
    ...
    body, surf = append_membrane(case, mesh, closed=cycled)
    body.general.bodyDim = 2
    ...
    if not cycled:
        ...
        om2e = case.om2edge.om2s.appendnew()
        om2e.nEdgePoint = nz * 2
        iz = np.arange(nz)
        ns = mesh.n2dpoint
        idx1 = iz * ns
        idx2 = (iz + 1) * ns - 1
        om2e.edgePoints = np.stack([idx1, idx2]).T.reshape((nz * 2, 1)) + 1

        return body, surf, om2e
    else:
        return body, surf, None
...
def append_plane_2d(c, vec, dx, xy0=None):
    mesh = create_plane_2d(vec, dx, xy0)
    return append_membrane_2d(c, mesh)
...
def append_stl_membrane(case, file, xyz=None, closed=False):
    mesh = TriSurface.from_stl(file)
    if xyz is not None:
        mesh.xyz.arr += np.array(xyz)
    return append_membrane(case, mesh, closed=closed)


def append_csv_membrane_2d(case, file, xy=None, cycled=False):
    csv = pd.read_csv(file)
    ...
    if xy is not None:
        curv += np.array(xy)[np.newaxis, :]
    return append_membrane_2d(case, curv, cycled=cycled)


def append_npz_membrane_2d(case, file, xy=None, cycled=False):
    curv = np.load(file)["xy"]
    if xy is not None:
        curv += np.array(xy)[np.newaxis, :]
    return append_membrane_2d(case, curv, cycled=cycled)
